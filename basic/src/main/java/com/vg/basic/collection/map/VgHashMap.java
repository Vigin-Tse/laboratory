 ...
 * 1.为什么会引入红黑树做查询优化呢？
 *
 * 但是当key为自定义的对象时，使HashMap中key的冲突率极高，为了保证高速的查找效率，就引入了红黑树来优化查询了。
 *
 * 2.为什么树化的临界值为8？
 *
 * 当桶中结点个数为8时，出现的几率十分的小（大概是亿分之6），因此常见的情况是桶中个数小于8的情况，此时链表的查询性能和红黑树相差不多，
 * 因为转化为树还需要时间和空间，所以此时没有转化成树的必要。
 *
 * 3.既然个数为8时发生的几率这么低，我们为什么还要当链表个数大于8时来树化来优化这几乎不会发生的场景呢？
 *
 * 首先我们要知道亿分之6这个几乎不可能的概率是建立在什么情况下的 答案是：建立在良好的hash算法情况下，例如String，Integer等包装类的hash算法、如果一旦发生桶中元素大于8，
 * 说明是不正常情况，可能采用了冲突较大的hash算法，此时桶中个数出现超过8的概率是非常大的，可能有n个key冲突在同一个桶中，此时再看链表的平均查询复杂度和红黑树的时间复杂度，
 * 就知道为什么要引入红黑树了，
 *
