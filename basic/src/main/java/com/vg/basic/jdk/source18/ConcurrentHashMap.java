...
package com.vg.basic.jdk.source18;
...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) { //自旋
            ...
            if (tab == null || (n = tab.length) == 0) //如果表示空的
                tab = initTable(); //初始化
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) { //如果下标 i 处（f为链表头部）原始为null
                if (casTabAt(tab, i, null,
                             new Node<K,V>(hash, key, value, null))) //直接用CAS操作，i处元素
                    break;                   // no lock when adding to empty bin
            }
            else if ((fh = f.hash) == MOVED) //如果下标 i 处元素不为null，且f.hash==MOVED，MOVED为常量
                tab = helpTransfer(tab, f);
            else {                           //如果是一般节点
                V oldVal = null;
                synchronized (f) {           //当头部元素不为null，切不需要装换成树时，需进行同步操作
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {
                            binCount = 1;
